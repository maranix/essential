  ...
  group('TaskGroup', () {
    group('State Derivation', () {
      ...
      test('should be active if any task is refreshing', () {
        final group = TaskGroup.uniform({
          '1': Task<int>.refreshing(),
          '2': Task<int>.success(data: 1),
        });
        expect(group.state, TaskGroupState.active);
      });
      ...
      test('should be failed if all tasks are failure', () {
        final group = TaskGroup.uniform({
          '1': Task<int>.failure(error: 'err1'),
          '2': Task<int>.failure(error: 'err2'),
        });
    ...
    group('Uniform TaskGroup', () {
      test('should add task', () {
        ...
        expect(group.getTask('1'), isA<Task<int>>());
      });

      test('should remove task', () {
        var group = TaskGroup<int>.uniform({'1': Task.pending()});
      ...
      test('should run all tasks', () async {
        ...
        expect(group.getTask<TaskSuccess<int>>('1')!.data, 1);
        expect(group.getTask<TaskSuccess<int>>('2')!.data, 2);
      });
    ...
    group('Mixed TaskGroup', () {
      test('should create mixed group', () {
        final group = TaskGroup.mixed({
          'user': Task<String>.pending(),
          'count': Task<int>.pending(),
        });
      ...
      test('should get typed task', () {
        ...
        expect(userTask, isA<Task<String>>());
      });
    });
    ...
    group('Extensions', () {
      test('should filter by label', () {
        final group = TaskGroup.uniform({
          '1': Task<int>.pending(label: 'a'),
          '2': Task<int>.pending(label: 'b'),
        });
      ...
      test('should preserve type after transition', () {
        final mixed = TaskGroup.mixed({'1': Task<int>.pending()});
    ...
    group('Type Safety & Getters', () {
      test('getTask<S> should return task if type matches', () {
        final group = TaskGroup.uniform({'1': Task<int>.success(data: 1)});
        final task = group.getTask<TaskSuccess<int>>('1');
      ...
      test('getTask<S> should throw TypeError if type mismatches', () {
        final group = TaskGroup.uniform({'1': Task<int>.success(data: 1)});
        expect(
          () => group.getTask<TaskFailure<int>>('1'),
      ...
      test('getTask<S> should return null if key missing', () {
        final group = TaskGroup.uniform({});
        expect(group.getTask<TaskSuccess<int>>('1'), isNull);
      });
      ...
      test('getter .failure should return error', () {
        final task = Task<int>.failure(error: 'oops');
