  ...
  group('Task', () {
    group('Factory Constructors', () {
      group('TaskPending', () {
        ...
        test('creates pending task with initialData', () {
          final task = Task<int, String?, Set<String>>.pending(initialData: 42);
          expect(task.isPending, isTrue);
      ...
      group('TaskRunning', () {
        test('creates running task with no parameters', () {
          final task = Task<int, String?, Set<String>>.running();
          expect(task.isRunning, isTrue);
          expect(task.initialData, isNull);
          expect(
            (task as TaskRunning<int, String?, Set<String>>).previousData,
            isNull,
          );
          expect(task.label, isNull);
          expect(task.tags, isNull);
        ...
        test('creates running task with initialData', () {
          final task = Task<int, String?, Set<String>>.running(initialData: 10);
          expect(task.initialData, equals(10));
        });

        test('creates running task with all parameters', () {
          final task = Task<String, String?, Set<String>>.running(
            initialData: 'initial',
          ...
          expect(
            (task as TaskRunning<String, String?, Set<String>>).previousData,
            equals('previous'),
          );
      ...
      group('TaskRefreshing', () {
        test('creates refreshing task with required previousData', () {
          final task = Task<int, String?, Set<String>>.refreshing(
            previousData: 42,
          );
          expect(task.isRefreshing, isTrue);
          expect(
            (task as TaskRefreshing<int, String?, Set<String>>).previousData,
            equals(42),
          );
        });

        test('creates refreshing task with all parameters', () {
          final task = Task<String, String?, Set<String>>.refreshing(
          ...
          expect(task.isRefreshing, isTrue);
          expect(
            (task as TaskRefreshing<String, String?, Set<String>>).previousData,
            equals('old-data'),
          );
          expect(task.initialData, equals('initial'));
      ...
      group('TaskRetrying', () {
        test('creates retrying task with required previousData', () {
          final task = Task<int, String?, Set<String>>.retrying(
            previousData: 42,
          );
          expect(task.isRetrying, isTrue);
          expect(
            (task as TaskRetrying<int, String?, Set<String>>).previousData,
            equals(42),
          );
        });

        test('creates retrying task with all parameters', () {
          final task = Task<double, String?, Set<String>>.retrying(
            previousData: 3.14,
            initialData: 2.71,
            label: 'retry-task',
          ...
          expect(task.isRetrying, isTrue);
          expect(
            (task as TaskRetrying<double, String?, Set<String>>).previousData,
            equals(3.14),
          );
      ...
      group('TaskSuccess', () {
        test('creates success task with required data', () {
          ...
          expect(
            (task as TaskSuccess<int, String?, Set<String>>).data,
            equals(42),
          );
        });

        test('creates success task with all parameters', () {
          final task = Task<String, String?, Set<String>>.success(
            data: 'result',
            initialData: 'initial',
            label: 'success-task',
          ...
          expect(task.isSuccess, isTrue);
          expect(
            (task as TaskSuccess<String, String?, Set<String>>).data,
            equals('result'),
          );
          expect(task.initialData, equals('initial'));
      ...
      group('TaskFailure', () {
        test('creates failure task with required error', () {
          ...
          final task = Task<int, String?, Set<String>>.failure(error: error);
          expect(task.isFailure, isTrue);
          expect(
            (task as TaskFailure<int, String?, Set<String>>).error,
            equals(error),
          );
        ...
        test('creates failure task with stackTrace', () {
          ...
          final task = Task<int, String?, Set<String>>.failure(
            error: error,
            stackTrace: stackTrace,
          );
          expect(
            (task as TaskFailure<int, String?, Set<String>>).stackTrace,
            equals(stackTrace),
          );
        });
        ...
        test('creates failure task with all parameters', () {
          ...
          final stackTrace = StackTrace.current;
          final task = Task<String, String?, Set<String>>.failure(
            error: error,
            stackTrace: stackTrace,
            previousData: 'old-data',
            initialData: 'initial',
          ...
          expect(task.isFailure, isTrue);
          final failure = task as TaskFailure<String, String?, Set<String>>;
    ...
    group('State Checking Getters', () {
      test('isPending returns true only for TaskPending', () {
        expect(Task<int, String?, Set<String>>.pending().isPending, isTrue);
        expect(Task<int, String?, Set<String>>.running().isPending, isFalse);
        expect(
          Task<int, String?, Set<String>>.refreshing(previousData: 1).isPending,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.retrying(previousData: 1).isPending,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.success(data: 1).isPending,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.failure(error: Exception()).isPending,
          isFalse,
        );
      });

      test('isRunning returns true only for TaskRunning', () {
        expect(Task<int, String?, Set<String>>.pending().isRunning, isFalse);
        expect(Task<int, String?, Set<String>>.running().isRunning, isTrue);
        expect(
          Task<int, String?, Set<String>>.refreshing(previousData: 1).isRunning,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.retrying(previousData: 1).isRunning,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.success(data: 1).isRunning,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.failure(error: Exception()).isRunning,
          isFalse,
        );
      });

      test('isRefreshing returns true only for TaskRefreshing', () {
        expect(Task<int, String?, Set<String>>.pending().isRefreshing, isFalse);
        expect(Task<int, String?, Set<String>>.running().isRefreshing, isFalse);
        expect(
          Task<int, String?, Set<String>>.refreshing(
            previousData: 1,
          ).isRefreshing,
          isTrue,
        );
        expect(
          Task<int, String?, Set<String>>.retrying(
            previousData: 1,
          ).isRefreshing,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.success(data: 1).isRefreshing,
          isFalse,
        );
        expect(
      ...
      test('isRetrying returns true only for TaskRetrying', () {
        expect(Task<int, String?, Set<String>>.pending().isRetrying, isFalse);
        expect(Task<int, String?, Set<String>>.running().isRetrying, isFalse);
        expect(
          Task<int, String?, Set<String>>.refreshing(
            previousData: 1,
          ).isRetrying,
        ...
        expect(
          Task<int, String?, Set<String>>.retrying(previousData: 1).isRetrying,
          isTrue,
        );
        expect(
          Task<int, String?, Set<String>>.success(data: 1).isRetrying,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.failure(
      ...
      test('isSuccess returns true only for TaskSuccess', () {
        expect(Task<int, String?, Set<String>>.pending().isSuccess, isFalse);
        expect(Task<int, String?, Set<String>>.running().isSuccess, isFalse);
        expect(
          Task<int, String?, Set<String>>.refreshing(previousData: 1).isSuccess,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.retrying(previousData: 1).isSuccess,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.success(data: 1).isSuccess,
          isTrue,
        );
        expect(
          Task<int, String?, Set<String>>.failure(error: Exception()).isSuccess,
          isFalse,
        );
      });

      test('isFailure returns true only for TaskFailure', () {
        expect(Task<int, String?, Set<String>>.pending().isFailure, isFalse);
        expect(Task<int, String?, Set<String>>.running().isFailure, isFalse);
        expect(
          Task<int, String?, Set<String>>.refreshing(previousData: 1).isFailure,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.retrying(previousData: 1).isFailure,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.success(data: 1).isFailure,
          isFalse,
        );
        expect(
          Task<int, String?, Set<String>>.failure(error: Exception()).isFailure,
          isTrue,
        );
      });
    });
    ...
    group('effectiveData Getter', () {
      ...
      test('returns previousData for TaskRunning', () {
        final task = Task<int, String?, Set<String>>.running(previousData: 42);
        expect(task.effectiveData, equals(42));
      });
      ...
      test('returns previousData for TaskRetrying', () {
        final task = Task<int, String?, Set<String>>.retrying(previousData: 42);
        expect(task.effectiveData, equals(42));
      });
    ...
    group('Convenience Transition Methods', () {
      group('toPending', () {
        test('transitions from any state to pending', () {
          final running = Task<int, String?, Set<String>>.running(
            previousData: 42,
          );
        ...
        test('preserves label and tags', () {
          final task = Task<int, String?, Set<String>>.success(
            data: 42,
            label: 'test',
            tags: {'tag1', 'tag2'},
          );
      ...
      group('toRunning', () {
        test('transitions from pending to running', () {
          final pending = Task<int, String?, Set<String>>.pending(
            initialData: 10,
          );
        ...
        test('preserves effectiveData as previousData', () {
          final success = Task<int, String?, Set<String>>.success(data: 42);
          final running = success.toRunning();

          expect(
            (running as TaskRunning<int, String?, Set<String>>).previousData,
            equals(42),
          );
        });
      ...
      group('toRefreshing', () {
        ...
        test('preserves data as previousData', () {
          ...
          expect(
            (refreshing as TaskRefreshing<int, String?, Set<String>>)
                .previousData,
            equals(42),
          );
        });
        ...
        test('scenario: refresh when initial load failed', () {
          ...
          final failure = Task<String, String?, Set<String>>.failure(
          ...
          expect(refreshing.isRefreshing, isTrue);
          expect(
            (refreshing as TaskRefreshing<String, String?, Set<String>>)
                .previousData,
            isNull,
          );
        ...
        test('scenario: refresh when nothing was displayed', () {
          ...
          final success = Task<List<String>, String?, Set<String>>.success(
          ...
          expect(refreshing.isRefreshing, isTrue);
          expect(
            (refreshing as TaskRefreshing<List<String>, String?, Set<String>>)
                .previousData,
            equals([]),
          );
      ...
      group('toRetrying', () {
        test('transitions from failure to retrying', () {
          final failure = Task<int, String?, Set<String>>.failure(
            error: Exception(),
            previousData: 10,
          );
        ...
        test('preserves previousData', () {
          ...
          expect(
            (retrying as TaskRetrying<int, String?, Set<String>>).previousData,
            equals(42),
          );
        });
        ...
        test('scenario: retry after failed fetch with no previous data', () {
          ...
          final failure =
              Task<Map<String, dynamic>, String?, Set<String>>.failure(
          ...
          expect(
            (retrying
                    as TaskRetrying<Map<String, dynamic>, String?, Set<String>>)
                .previousData,
            isNull,
          );
      ...
      group('toSuccess', () {
        test('transitions from running to success', () {
          ...
          expect(
            (success as TaskSuccess<int, String?, Set<String>>).data,
            equals(42),
          );
        });
        ...
        test('preserves label, tags, and initialData', () {
          ...
          expect(
            (success as TaskSuccess<String, String?, Set<String>>).data,
            equals('result'),
          );
      ...
      group('toFailure', () {
        test('transitions from running to failure', () {
          ...
          expect(
            (failure as TaskFailure<int, String?, Set<String>>).error,
            equals(error),
          );
        });
        ...
        test('preserves effectiveData as previousData', () {
          ...
          expect(
            (failure as TaskFailure<int, String?, Set<String>>).previousData,
            equals(42),
          );
        });
        ...
        test('includes stackTrace when provided', () {
          ...
          expect(
            (failure as TaskFailure<int, String?, Set<String>>).stackTrace,
            equals(stackTrace),
          );
        });
        ...
        test('preserves label, tags, and initialData', () {
          ...
          expect(
            (failure as TaskFailure<String, String?, Set<String>>).previousData,
            equals('old'),
          );
      ...
      group('state getter', () {
        test('returns correct state for each task type', () {
          expect(
            Task<int, String?, Set<String>>.pending().state,
            equals(TaskState.pending),
          );
          expect(
            Task<int, String?, Set<String>>.running().state,
            equals(TaskState.running),
          );
          expect(
            Task<int, String?, Set<String>>.refreshing(previousData: 1).state,
            equals(TaskState.refreshing),
          );
          expect(
            Task<int, String?, Set<String>>.retrying(previousData: 1).state,
            equals(TaskState.retrying),
          );
          expect(
            Task<int, String?, Set<String>>.success(data: 1).state,
            equals(TaskState.success),
          );
          expect(
            Task<int, String?, Set<String>>.failure(error: Exception()).state,
    ...
    group('mapData Method', () {
      test('transforms data in TaskSuccess', () {
        ...
        expect(
          (transformed as TaskSuccess<int, String?, Set<String>>).data,
          equals(84),
        );
      });
      ...
      test('transforms data type in TaskSuccess', () {
        ...
        expect(transformed.isSuccess, isTrue);
        expect(
          (transformed as TaskSuccess<String, String?, Set<String>>).data,
          equals('42'),
        );
      });

      test('transforms previousData in TaskRunning', () {
        final task = Task<int, String?, Set<String>>.running(previousData: 10);
        final transformed = task.mapData((data) => data * 3);

        expect(transformed.isRunning, isTrue);
        expect(
          (transformed as TaskRunning<int, String?, Set<String>>).previousData,
          equals(30),
        );
      });
      ...
      test('transforms previousData in TaskRefreshing', () {
        ...
        expect(
          (transformed as TaskRefreshing<String, String?, Set<String>>)
              .previousData,
          equals('HELLO'),
        );
      });
      ...
      test('transforms previousData in TaskRetrying', () {
        ...
        expect(
          (transformed as TaskRetrying<int, String?, Set<String>>).previousData,
          equals(3),
        );
      });
      ...
      test('transforms previousData in TaskFailure', () {
        ...
        expect(
          (transformed as TaskFailure<int, String?, Set<String>>).previousData,
          equals(50),
        );
      });
      ...
      test('preserves error in TaskFailure', () {
        ...
        expect(
          (transformed as TaskFailure<int, String?, Set<String>>).error,
          equals(error),
        );
      });

      test('transforms initialData in TaskPending', () {
        final task = Task<int, String?, Set<String>>.pending(initialData: 5);
    ...
    group('mapError Method', () {
      ...
      test('preserves stackTrace in TaskFailure', () {
        ...
        expect(
          (transformed as TaskFailure<int, String?, Set<String>>).stackTrace,
          equals(stackTrace),
        );
      });
      ...
      test('returns same task for non-failure states', () {
        ...
        final refreshing = Task<int, String?, Set<String>>.refreshing(
          previousData: 1,
        );
        expect(refreshing.mapError((e) => Exception('New')), same(refreshing));

        final retrying = Task<int, String?, Set<String>>.retrying(
          previousData: 1,
        );
    ...
    group('transform Method', () {
      ...
      test('updates initialData in TaskPending', () {
        final task = Task<int, String?, Set<String>>.pending(initialData: 10);
        final transformed = task.transform(
    ...
    group('copyWith Method', () {
      ...
      test('copies TaskPending with new label', () {
        final task = Task<int, String?, Set<String>>.pending(label: 'old');
      ...
      test('copies TaskPending with new tags', () {
        final task = Task<int, String?, Set<String>>.pending(tags: {'old'});
      ...
      test('copies TaskRunning with new previousData', () {
        ...
        expect(
          (copied as TaskRunning<int, String?, Set<String>>).previousData,
          equals(20),
        );
      });
      ...
      test('copies TaskRefreshing with new previousData', () {
        ...
        expect(
          (copied as TaskRefreshing<String, String?, Set<String>>).previousData,
          equals('new'),
        );
      });
      ...
      test('copies TaskRetrying with new previousData', () {
        ...
        expect(
          (copied as TaskRetrying<int, String?, Set<String>>).previousData,
          equals(200),
        );
      });
      ...
      test('copies TaskSuccess with new data', () {
        ...
        expect(
          (copied as TaskSuccess<int, String?, Set<String>>).data,
          equals(84),
        );
      });
      ...
      test('copies TaskSuccess preserving existing values', () {
        ...
        expect(
          (copied as TaskSuccess<String, String?, Set<String>>).data,
          equals('result'),
        );
      ...
      test('copies TaskFailure with new error', () {
        ...
        expect(
          (copied as TaskFailure<int, String?, Set<String>>).error,
          equals(newError),
        );
      });
      ...
      test('copies TaskFailure with new stackTrace', () {
        ...
        expect(
          (copied as TaskFailure<int, String?, Set<String>>).stackTrace,
          equals(newStackTrace),
        );
      });
      ...
      test('copies TaskFailure with new previousData', () {
        ...
        expect(
          (copied as TaskFailure<int, String?, Set<String>>).previousData,
          equals(20),
        );
      });
      ...
      test('copies TaskFailure preserving existing values', () {
        ...
        final failure = copied as TaskFailure<int, String?, Set<String>>;
        expect(failure.error, equals(error));
        expect(failure.stackTrace, equals(stackTrace));
    ...
    group('Edge Cases and Complex Scenarios', () {
      test('handles complex type transformations', () {
        final task = Task<List<int>, String?, Set<String>>.success(
          data: [1, 2, 3],
        );
      ...
      test('handles nullable types correctly', () {
        final task = Task<int?, String?, Set<String>>.success(data: null);
        expect((task as TaskSuccess<int?, String?, Set<String>>).data, isNull);
      });

      test('preserves empty tags set', () {
        final task = Task<int, String?, Set<String>>.pending(tags: {});
      ...
      test('effectiveData prioritizes correctly in complex scenario', () {
        ...
        final task2 = Task<int, String?, Set<String>>.running(initialData: 50);
        expect(task2.effectiveData, equals(50));

        final task3 = Task<int, String?, Set<String>>.running();
        expect(task3.effectiveData, isNull);
      });

      test('convenience methods preserve all metadata', () {
        final oldTask = Task<int, String?, Set<String>>.success(
          data: 42,
          initialData: 10,
          label: 'complete',
        ...
        expect(
          (newTask as TaskRefreshing<int, String?, Set<String>>).previousData,
          equals(42),
        );
      ...
      test('chaining multiple transitions', () {
        ...
        expect(
          (result as TaskRefreshing<int, String?, Set<String>>).previousData,
          equals(20),
        );
      ...
      test('copyWith on TaskSuccess maintains type safety', () {
        final task = Task<String, String?, Set<String>>.success(data: 'hello');
      ...
      test('transform with multiple update functions', () {
        ...
        final failure = transformed as TaskFailure<int, String?, Set<String>>;
      ...
      test('preserves const tags across operations', () {
        ...
        final task = Task<int, String?, Set<String>>.pending(tags: tags);
    ...
    group('Type Safety and Generics', () {
      test('works with custom class types', () {
        final task = Task<_TestData, String?, Set<String>>.success(
          data: _TestData('test', 42),
        );
        expect(task.isSuccess, isTrue);
        expect(
          (task as TaskSuccess<_TestData, String?, Set<String>>).data.name,
          equals('test'),
        );
      ...
      test('transforms custom types correctly', () {
        ...
        final result =
            transformed as TaskSuccess<_TestData, String?, Set<String>>;
        expect(result.data.name, equals('ORIGINAL'));
        expect(result.data.value, equals(20));
      });

      test('handles List types', () {
        final task = Task<List<String>, String?, Set<String>>.success(
          data: ['a', 'b', 'c'],
        );
        final transformed = task.mapData(
        ...
        expect(
          (transformed as TaskSuccess<List<String>, String?, Set<String>>).data,
          equals(['A', 'B', 'C']),
        );
      });

      test('handles Map types', () {
        final task = Task<Map<String, int>, String?, Set<String>>.success(
          data: {'a': 1, 'b': 2},
        );
        final transformed = task.mapData((data) => data.length);
      ...
      test('handles nested generic types', () {
        final task = Task<List<Map<String, int>>, String?, Set<String>>.success(
          data: [
            {'a': 1},
            {'b': 2},
          ],
        ...
        expect(task.isSuccess, isTrue);
        expect(
          (task as TaskSuccess<List<Map<String, int>>, String?, Set<String>>)
              .data
              .length,
          equals(2),
        );
    ...
    group('Static Methods', () {
      group('runSync', () {
        test('returns success on successful execution', () {
          final task = Task.runSync<int, String?, Set<String>>(() => 42);
          expect(task.isSuccess, isTrue);
          expect(
            (task as TaskSuccess<int, String?, Set<String>>).data,
            equals(42),
        ...
        test('returns failure on error', () {
          ...
          final task = Task.runSync<int, String?, Set<String>>(
            () => throw error,
          );
          expect(task.isFailure, isTrue);
      ...
      group('run', () {
        test('returns TaskSuccess on successful async execution', () async {
          final task = await Task.run<String, String?, Set<String>>(() async {
      ...
      group('watch', () {
        test('emits running then success', () {
          final stream = Task.watch<int, String?, Set<String>>(() async {
            await Future<void>.delayed(const Duration(milliseconds: 10));
            return 100;
          });
          ...
          expect(
            stream,
            emitsInOrder([
              isA<TaskRunning<int, String?, Set<String>>>(),
              isA<TaskSuccess<int, String?, Set<String>>>().having(
                (t) => t.data,
                'data',
                100,
              ),
        ...
        test('emits running then failure on error', () {
          ...
          expect(
            ...
            emitsInOrder([
              isA<TaskRunning<int, String?, Set<String>>>(),
              isA<TaskFailure<int, String?, Set<String>>>().having(
                (t) => t.error,
                'error',
                error,
              ),
